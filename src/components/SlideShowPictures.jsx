...
export default function PictureCard({picture, picturepath, selectedTab}) {

    const [isEditable, setEditable] = useState(false)
    const [ name , setName] = useState(picture.title)
   ...
   <div className="p-1 bg-gradient-to-tr from-stone-900 border-white border-2 border-dotted flex flex-col items-center  m-4  ">
    ...
    bg-gradient-to-r from-orange-500 to-orange-200 hover:from-orange-600 hover:to-orange-300 flex underline underline-offset-4"> {name} </p>}
     { isEditable && <input type="text"  className="text-black" value={name} onChange={(e) => setName(e.target.value)} />}
